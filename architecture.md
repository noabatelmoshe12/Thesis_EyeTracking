...
3.  **Primary Analysis (MATLAB)**: The `Convert_eye_data.m` function maps raw gaze coordinates to specific Areas of Interest (AOIs) blocks (Header, Attributes, Alternatives) to generate a sequence of fixations.
...
+-- Convert_eye_data.m      # Core MATLAB analysis logic (converts gaze to AOIs)
...
*   **`process_eye_data.py`**:
    ...
    *   **Function**: Finds all `Subject_*` files in `Eyedata/`, starts the MATLAB Engine API for Python, and calls `Convert_eye_data` for every subject.
...
*   **Script**: `C:\Users\user\Documents\Noa\Thesis_EyeTracking\Convert_eye_data.m`
...
| **3** | `Convert_eye_data.m` | **Extract Fixations** | `.../Eyedata/Subject_XX_eyeData.mat`<br>*(Reads `edfStruct` variable)* | `.../function/XX.csv`<br>*(Contains fixation sequence: A1, B2, etc.)* |
...
graph TD
    ...
    C -->|Input to| D[Convert_eye_data.m]
