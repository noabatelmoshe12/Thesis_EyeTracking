...
def summarize_subject_vertical_index(subject_id, csv_path):
    """
    Create one merged vertical scan index per subject across both blocks.
    ...
    project_root = Path(r"C:\Projects\Thesis_EyeTracking")
    csv_dir = project_root / "data" / "processed" / "fixations"
    output_dir = project_root / "data" / "results" / "merged_3_4"
    output_file = output_dir / "subject_vertical_index_merged_blocks.csv"
    ...
    for csv_path in csv_files:
        ...
        summary_df = summarize_subject_vertical_index(subject_id, csv_path)
    ...
    print(f"\nSaved merged subject-level vertical index to:\n{output_file}")
    print("\nPreview:")
