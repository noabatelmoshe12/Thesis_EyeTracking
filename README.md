...
*    - `data/results/block_results_summary.csv` — block-level metrics for all subjects (the main summary used for statistical analysis).
...
2. Aggregates and updates `data/results/trial_results_summary.csv` (trial-level) and `data/results/block_results_summary.csv` (block-level) with the newly calculated `Scanpath_Index` scores for statistical analysis.
